...
:
    ...
    wallPhi_("wallPhi", dimless, coeffDict_),
    viscosityFactor_("viscosityFactor", dimless, coeffDict_)
{
    phi_ *= constant::mathematical::pi/180.0;
    wallPhi_ *= constant::mathematical::pi/180.0;
}
...
) const
{
    ...
    forAll(D, celli)
    {
        ...
        {
            nuf[celli] =
                viscosityFactor_.value()*0.5*pf[celli]*sin(phi_.value())
               /(
        ...
        if (debuggingKineticTheory)
        {
            ...
            Info<< "wall angle of friction: " << wallPhi_.value() << endl;
            Info<< "internal angle of friction: " << phi_.value() << endl;
        }
        ...
        {   
            ...
            forAll(patches, patchi)
            {
                if (!patches[patchi].coupled())
                {
                    nufBf[patchi] =
                        (
                            pf.boundaryField()[patchi]*sin(wallPhi_.value())
                            /(
...
JohnsonJacksonSchaeffer::read()
{
    ...
    wallPhi_.read(coeffDict_);
    wallPhi_ *= constant::mathematical::pi/180.0;
    
    alphaDeltaMin_.read(coeffDict_);
    
    viscosityFactor_.read(coeffDict_);

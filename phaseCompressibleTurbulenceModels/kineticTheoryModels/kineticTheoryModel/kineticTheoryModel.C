...
:
    ...
    particleParticleGranularDissipation_(coeffDict_.lookup("particleParticleGranularDissipation")),
    fluidParticleGranularDissipation_(coeffDict_.lookup("fluidParticleGranularDissipation")),
    debuggingKineticTheory_(coeffDict_.lookup("debuggingKineticTheory")),
    wallFriction_(coeffDict_.lookup("wallFriction")),
    pf_
    (
        IOobject
        (
...
void Foam::RASModels::kineticTheoryModel::correct()
{
    ...
    if (!equilibrium_)
    {
        ...
        if (debuggingKineticTheory_)
        {
            
            Info << "particleParticleGranularDissipation = " << particleParticleGranularDissipation_ << endl;
            Info << "max(gammaCoeff) before = " << gMax(gammaCoeff) << endl;
            Info << "min(gammaCoeff) before = " << gMin(gammaCoeff) << endl;   
        }    
        
        if (particleParticleGranularDissipation_ == "JenkinsSavage")
        {
            ...
            (
                3.0*(1.0 - sqr(e_))
                *max(sqr(alpha), residualAlpha_)
                *rho*gs0_*( (4.0/da)*ThetaSqrt/sqrtPi - fvc::div(U) )
            );
        ...
        if (debuggingKineticTheory_)
        {
            ...
            Info << "max(gammaCoeff) after = " << gMax(gammaCoeff) << endl;
            Info << "min(gammaCoeff) after = " << gMin(gammaCoeff) << endl;
        }
        ...
        if (debuggingKineticTheory_)
        {
            Info << "fluidParticleGranularDissipation = " << fluidParticleGranularDissipation_ << endl;
            Info<< "max(J2) before: " << gMax(J2) << endl;
            Info<< "min(J2) before: " << gMin(J2) << endl;
        }

        if (fluidParticleGranularDissipation_ == "Koch")
        {
            
            const volScalarField nuc_ =
            refCast<const twoPhaseSystem>(phase_.fluid()).otherPhase(phase_).nu();
            
            const volScalarField rhoc_ =
            refCast<const twoPhaseSystem>(phase_.fluid()).otherPhase(phase_).rho();
            
            J2 = 81.0*max(alpha, residualAlpha_)*sqr(nuc_*rhoc_)*magSqr(U - Uc_)
                /(
                    gs0_*(pow(da,3.0))*rho_*sqrtPi*(ThetaSqrt + ThetaSmallSqrt)
                );     
        ...
        if (debuggingKineticTheory_)
        {
            Info<< "max(J2) after: " << gMax(J2) << endl;
            Info<< "min(J2) after: " << gMin(J2) << endl;
        }
